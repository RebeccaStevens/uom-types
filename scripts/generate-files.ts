...
const mut_exponents = new Set<number>();
...
function generateSiUnitPrefixesFile() {
  ...
  const main = [...mut_exponents.values()]
    .map((exponent) => {
...
function populateExponents() {
  for (const exponent of Array.from({ length: maxExponent + 1 }, (_, i) => i)) {
    ...
    mut_exponents.add(exponent);
    mut_exponents.add(-exponent);
  }
}

function getExponentTypeDefintion() {
  const exponentsArray = [...mut_exponents.values()];
...
function getSumExponentsTypeDefintion() {
  return `/**\n * @group Exponent Functions\n */\nexport type SumExponents<A extends Exponent, B extends Exponent> = ${[
    ...mut_exponents.values(),
  ]
    .map(
      (a) =>
        `A extends ${a} ? ${[...mut_exponents.values()]
          .map((b) => {
            const sum = a + b;
            return mut_exponents.has(sum) ? `B extends ${b} ? ${sum} :` : null;
          })
          .filter(isNotNull)
          .join(" ")} never :`,
    )
    ...
    .map(
      (a) =>
        `A extends ${a} ? ${[...mut_exponents.values()]
          .map((b) => {
            const product = a * b;
            return mut_exponents.has(product) ? `B extends ${b} ? ${product} :` : null;
          })
          .filter(isNotNull)
          .join(" ")} never :`,
    )
...
function getDivideExponentsTypeDefintion() {
  return `/**\n * @group Exponent Functions\n */\nexport type DivideExponents<A extends Exponent, B extends Exponent> = ${[
    ...
    .map(
      (b) =>
        `B extends ${b} ? ${[...mut_exponents.values()]
          .map((a) => {
            const result = a / b;
            return mut_exponents.has(result) ? `A extends ${a} ? ${result} :` : null;
          })
          .filter(isNotNull)
          .join(" ")} never :`,
    )
