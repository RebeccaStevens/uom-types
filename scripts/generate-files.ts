...
function generateSiUnitPrefixesFile() {
  const imports = `import { type MultiplyUnit, type UnitCore, type UnknownUnit } from "#uom-types";\n\n`;
  const main = [...exponents.values()]
    .map((exponent) => {
      ...
      return `/**\n * Binary prefix denoting an orders of magnitude of 10^${exponent}.\n *\n * @siprefix\n */\nexport type ${name}<T extends UnknownUnit> = MultiplyUnit<T, UnitCore<{}, { scalar10: ${exponent} }>>;`;
    })
    .filter(isNotNull)
    .join("\n\n");
